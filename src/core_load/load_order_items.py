    ...
    try:
        ...
        for row in rows:
            ...
            if order_item_result is None:
                ...
                (order_item_id, order_key, product_key, quantity, price)
                VALUES (%s, %s, %s, %s, %s);""",
                               (row[0], order_key, product_key, row[3], row[4]))
                ...
                WHERE order_item_id = %s;
                """,
                               (order_key, product_key, row[3], row[4], row[0]))
        ...
        logging.exception(f"Load_order_items failed")
