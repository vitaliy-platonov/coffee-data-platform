    ...
    try:
        ...
        for row in rows:
            ...
            if payment_result is None:
                ...
                (payment_id, order_key, payment_date, payment_method, amount, status)
                VALUES (%s, %s, %s, %s, %s, %s);""",
                               (row[0], order_key, row[2], row[3], row[4], row[5]))
                ...
                WHERE payment_id = %s;
                """,
                               (order_key, row[2], row[3], row[4], row[5], row[0]))
        ...
        logging.exception(f"Load_payments failed")
