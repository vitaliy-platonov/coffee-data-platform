...
def load_orders():
    ...
    try:
        ...
        for row in rows:
            ...
            if order_result is None:
                ...
                INSERT INTO core.fact_orders
                    (order_id, customer_key, store_key, employee_key, order_date, total_amount)
                VALUES (%s, %s, %s, %s, %s, %s);""",
                               (row[0], customer_key, store_key, employee_key, row[4], row[5]))
                ...
                total_amount = %s
                WHERE order_id = %s;""",
                               (customer_key, store_key, employee_key, row[4], row[5], row[0]))
        ...
        logging.exception(f"Load_orders failed")
